...
$CONC = if ($env:BENCH_CONC) { [int]$env:BENCH_CONC } else { 50 }
$DUR  = if ($env:BENCH_DUR)  { [int]$env:BENCH_DUR }  else { 8 }
...
function Run-Load($url) {
    $out = & $PY $LOADTEST $url $CONC $DUR 2>$null
...
Write-Host "  concurrency=$CONC duration=${DUR}s/endpoint" -ForegroundColor Cyan
Write-Host "=================================================================" -ForegroundColor Cyan
Write-Host ""
...
$md.Add("- **Load:** $CONC keep-alive connections x ${DUR}s per endpoint")
