...
if ($hasFast) {
    ...
    $proc = Start-Process -FilePath $PY -WorkingDirectory "$BENCH\fastapi_app" `
        -ArgumentList "-m","uvicorn","main:app","--host","127.0.0.1","--port","8400","--workers","8","--no-access-log","--log-level","warning" `
...
$md.Add("- **Load:** $CONC keep-alive connections x $REQUESTS requests per endpoint  (8 proc x 1 worker each)")
